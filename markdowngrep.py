...
def parseCommandLine():
    parser = argparse.ArgumentParser(prog='markdowngrep',
        description='Searches markdown-formatted text files for a given '
                    'pattern/string and, '
                    'given a match; returns the heading that contains the '
                    'text.',
        epilog='')
...
def processInput(input, pattern):
    ...
    for num, line in enumerate(input_data):
        if line.strip():
            if pattern in line:
                log.debug('Found match: [{number}] {content}'.format(number=num,
                    content=line.strip()))
                ...
                while i > 0:
                    ...
                    if sr_h:
                        log.info('{h} ... {t}'.format(h=sr_h.group(2),
                                                      t=pattern))
                        break
                    i -= 1
                ...
                while i > 0:
                    ...
                    if sr_lh:
                        log.info('{h} ... {t}'.format(h=sr_lh.group(2),
                                                      t=pattern))
                        break
                    i -= 1
    return
